...
public class ex111 {
    public static void main(String[] args) {
        ...
        //System.out.println(nullValue.toLowerCase()); // Exception 발생!
        ...
        // orElse() 함수
        String strResult = Optional.ofNullable(normalValue).orElse("Null이군요");
            System.out.println(strResult);

        // filter()함수
        // : 람다식이 true면 Optional 객체값이 그대로 전달되고, false이면, orElse에 정의된 값이 전달
        String strResult2 = Optional.ofNullable(normalValue)
                .filter(/* 람다식 */ (val) -> val.contains("abc"))
                .orElse("abc값 없네요");
        System.out.println(strResult2);

        // map()함수
        // String::toUpperCase: 람다식의 간결한 표현
        String strResult3 = Optional.ofNullable(nullValue)
